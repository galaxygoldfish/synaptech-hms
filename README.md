...
- Node.js
- npm
- A Supabase project with the `profiles`, `loans`, `equipment`, and `equipment_units` tables (see [Database](#database) below)
...
- `equipment` — a catalog entry (name, description, category, image)
- `equipment_units` — a physical, serialized unit of an `equipment` entry
- `loans` — a member's checkout of a specific `equipment_units` row, with a `status` lifecycle (`pending_checkout` → `active`/`overdue` → `pending_return` → `returned`, or `cancelled`)

Row-level security restricts each table to the rows a user is allowed to see/edit (e.g. members can only read their own `loans` rows; only the profile owner can read/write their own `profiles` row).
